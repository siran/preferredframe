...
%     perfil = double(curve(1:size(perfil)));
    cuentaPicos = 0;
    distanciaEntrePicos = 10;
    for k=2+distanciaEntrePicos:size(perfil)-1-distanciaEntrePicos
        if perfil(k)>=perfil(k-1) && perfil(k)>=perfil(k+1)
            if k<locs(cuentaPicos)+distanciaEntrePicos
                continue
            end
            if (perfil(k) = max(perfil(k-distanciaEntrePicos:k+distanciaEntrePicos)))
