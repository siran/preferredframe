...
%     g=1;
%     arrastre = (1-1/n^2 - f*cos(theta)*dndl*lambda/n);
%     arrastre = (1-1/n^2);
    
    

%     lambdap(i) = (c/n + v*cos(theta)*(1-1/n^2))/(c/n)*lambda;
%     dl = lambdap(i)-lambda;
%     dn = dndl*dl;
%     arrastre = (1-1/(n+dn)^2);
...
%     arrastre = v*(1-1/n^2);
%     arr_long = cos(theta)*arrastre;
%     arr_trans = sin(theta)*arrastre;
      %  - abs(cos(theta))*lambda/n*dndl)
    eqt = -(c/n*t)^2 ...
        + (g*L + ( t*v*arrastre))^2;% ...
%         + (      sin(theta)*(t*v - t*v*arrastre))^2;
%     b = (1 - 1/n^2)*v*t*(sin(theta));
    % b = (1 - 1/n^2)*v*t*(sin(theta) + atran);      
    % b = (1 - 1/n^2 + doppler*lambda/n*dndl)*v*t*(sin(theta) + atran);      
%     b = (1-1/n^2)*v*sin(theta)*t;  
    % b = (1-1/n^2)*v*t;   
%     b = (1 - 1/n^2 )*v*t*(1);
% %     b=0;
