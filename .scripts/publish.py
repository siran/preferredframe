...
Flow:
  ...
  6) Reserve DOI; build permalink from reserved DOI
  7) Preview → single confirmation
  8) Commit (publication)
  9) Copy PDF to assets repo (default ../assets) & push (default ON); mirror PDF into site/assets
 10) Upload files; publish (reserved DOI becomes final); sanity check
 11) Auto push branch and merge into main on success
"""
...
def zenodo_api_and_token() -> Tuple[str, str]:
    # prefer prod token, else sandbox
...
def yaml_str(v: str) -> str:
    # quote only if necessary (leading/trailing spaces or newlines)
    ...
    if isinstance(obj, list):
        ...
        for it in obj:
            if isinstance(it, dict):
                # render "- key: val" then aligned subsequent keys
                first = True
                for k, v in it.items():
                    if first:
...
def write_provenance(dst_dir: Path, dst_md: Path, dst_pdf: Path, dst_html: Path, dst_pandoc_md: Path,
                     ...
                     reserved_doi: str, concept_doi: str,
                     assets_pdf_url: str, site_html_url: str) -> Path:
    provenance = {
        ...
        "zenodo": {
            "deposition_id": reserved_doi.split("/")[-1] if reserved_doi else "",
...
def main():
    ...
    # ---- branch, copy, render ----
    dst_dir, dst_md, dst_pdf, date_str, branch_name = prepare_paths_and_branch(src_md, site_repo, src_commit)
    ...
    # permalink (human-facing) from reserved DOI
    ...
    for p in [dst_md, dst_html, dst_pandoc_md]:
        echo(f" - {p.relative_to(site_repo)}")
    echo(" - prints/.../provenance.yaml")
    ...
    prov_path = write_provenance(dst_dir, dst_md, dst_pdf, dst_html, dst_pandoc_md,
                                 ...
                                 reserved_doi, concept_doi, assets_pdf_url, site_html_url)
    ...
    # ---- copy PDF to assets & push (default ON) + mirror into site/assets ----
    if args.assets_dir:
        ...
        # mirror into local site/ to mimic production
    ...
    if final_doi_reported and final_doi_reported != reserved_doi:
        echo("WARNING: final DOI differs from reserved DOI.")

    # ---- push branch & merge to main (auto) ----
