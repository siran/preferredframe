...
- Build .pnp.md, .html, .pdf; write print.json; commit/push
- Optional DOI; emit artifact_path for workflow
"""
...
def run(args):
    r = sh(args); sys.stdout.write(r.stdout); return r

def find_single_md() -> Path:
    mds = list(PRINTS.glob("*/*.md"))
    if not mds:
        print("[print][ERROR] No .md under prints/*/"); sys.exit(1)
    if len(mds) > 1:
        print("[print][ERROR] Multiple .md files found:"); [print(" -", m.relative_to(ROOT)) for m in mds]; sys.exit(1)
    return mds[0]
...
def build(md: Path):
    ...
    for f in (md, pnp, html, pdf):
        if not f.exists() or f.stat().st_size == 0:
            print(f"[print][ERROR] Missing/empty: {f.relative_to(ROOT)}"); sys.exit(1)
...
def maybe_mint_doi(target_dir: Path) -> str | None:
    ...
    if not token:
        print("[print] ZENODO_ACCESS_TOKEN not set; skipping DOI."); return None
    zp = ROOT / ".scripts" / "src" / "zenodo_publish.py"
    if not zp.exists():
        print("[print] zenodo_publish.py not found; skipping DOI."); return None
    try:
...
def git_commit_and_push(paths: list[Path], msg: str):
    ...
    if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=ROOT).returncode == 0:
        print("[print] No changes to commit."); return
...
def main():
    ...
    if out:
        with open(out, "a", encoding="utf-8") as fh:
            print(f"artifact_path={md.parent.relative_to(ROOT)}", file=fh)
