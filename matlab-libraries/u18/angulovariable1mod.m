...
h = L1*(cos(t3).^(-1));
% h1   = L1*g_theta.^(-1).*(c_theta_aire).^(-1);
...
correccion = L2+L1*(tan(t3)-tan(t1))/c + L1/c*;
...
final=delta_t+correccion;
final=final-final(1);

figure
plot(theta_deg, final)
title('doble chepa original')

return

figure
plot(theta_deg, (delta_t + 0.005*correccion)*tiempo2franjas)
title('doble chepa - correccion')
