...
r0 = [0,0,0] %#ok<*NOPTS>
r1 = [vx*t1, vy*t1, vz*t1] + [0,gy*L1, 0]
r2 = [vx*t2, vy*t2, vz*t2] + [gx*L2, 0, 0]
r3 = [vx*t3, vy*t3, vz*t3] + [0, -gy*L1, 0]
...
angulos = 0;
vzn=sin(0:pi/400:pi);
vxn=vzn.*cos(angulos)*300;
vyn=vzn.*sin(angulos)*300;


% aqui evaluo numericamente las expresiones simbolicas. 'subs' es la
% funcion para 'sustituir'
t1V = eval(subs(subs(t1s, {L1, L2, gx, gy}, {.3/1000, .6/1000, 1/abs(sqrt(1-(vx/c)^2)), 1/abs(sqrt(1-(vy/c)^2))}),{vx,vy,vz, c},{vxn,vyn,vzn,300000}));
t2V = eval(subs(subs(t2s, {L1, L2, gx, gy}, {.3/1000, .6/1000, 1/abs(sqrt(1-(vx/c)^2)), 1/abs(sqrt(1-(vy/c)^2))}),{vx,vy,vz, c},{vxn,vyn,vzn,300000}));
t3V = eval(subs(subs(t3s, {L1, L2, gx, gy}, {.3/1000, .6/1000, 1/abs(sqrt(1-(vx/c)^2)), 1/abs(sqrt(1-(vy/c)^2))}),{vx,vy,vz, c},{vxn,vyn,vzn,300000}));
t4V = eval(subs(subs(t4s, {L1, L2, gx, gy}, {.3/1000, .6/1000, 1/abs(sqrt(1-(vx/c)^2)), 1/abs(sqrt(1-(vy/c)^2))}),{vx,vy,vz, c},{vxn,vyn,vzn,300000}));
...
for n=1:size(t4V)
    d = 300000*t4V(n)./cos(0/360*2*pi:pi/100000:20/360*2*pi);
    w = 632*10^9;
    plot(sin(w*d/300000));
