...
for ang=0:5:360
    ...
    eq = (c*t)^2-(g*L)^2-(a+b)^2;
    tv(i) = solve(eq, 'real', true);
    
%     n = n_aire;    
%     a = g*L*tan(t1);
%     b = (1-1/n^2)*v*t;
%     
%     eq = (c/n*t)^2-(g*L)^2-(a+b)^2;
%     ta(i) = solve(eq, 'real', true);    
%     
%     diff(i) = tv(i)-ta(i);
end

% plot(theta_deg, (diff-diff(1))*tiempo2franjas)
% title('Fringe displacement v. angle of rotation')
% xlabel('Angle of rotation (º)')
% ylabel('Fringe displacement')

figure
plot(theta_deg, (tv-tv(1))*tiempo2franjas)
title('Time of flight glass')
xlabel('Angle of rotation (º)')
ylabel('Time of flight - glass')

% figure
% plot(theta_deg, (ta-ta(1))*tiempo2franjas)
% title('Time of flight - air')
% xlabel('Angle of rotation (º)')
% ylabel('Time of flight - air')
